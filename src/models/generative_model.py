...
class Decoder_FC(nn.Module):
    def __init__(self, x_dim, latent_dim):
        """
        Fully connected decoder for mapping latent space to original space.
        """
    ...
    def log_features(self, features, step):
        """
        Log feature statistics to wandb.
        """
        mean_features = torch.mean(features, dim=0).cpu().numpy()
        wandb.log({f"Feature Mean at Step {step}": mean_features.tolist()})
